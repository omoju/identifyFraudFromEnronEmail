...
To zoom in on the compensation, we focused on two features `exercised\_stock\_options' and `restricted\_stock'. Table \ref{exStTable} shows the employees with the highest exercised stock options. We see that the sitting CEO, Ken Lay, had the highest exercised stock option, that wasn't surprising. What was surprising was the name ``Pai Lou.'' From descriptive statistics on salary, 
